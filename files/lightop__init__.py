...
def redirectme():
    ...
    if os.environ.get('NVIDIA_OPENGL', '').lower() in ('true', 't', '1', 'y', 'yes'):
        subprocess.check_call(r"DISPLAY=:1 xrandr --fb {width}x{height}".format(**env), shell=True)
        req = "5"
    else:
        # sed
        subprocess.check_call(
            "DISPLAY=:1 xrandr --newmode {width}x{height} `gtf {width} {height} 60|grep Modeline|grep Modeline|sed 's/Modeline  *\".*\" *//'`"
            .format(**env), shell=True)
        subprocess.check_call("DISPLAY=:1 xrandr --addmode screen {width}x{height}".format(**env), shell=True)
        subprocess.check_call("DISPLAY=:1 xrandr --output screen --mode {width}x{height}".format(**env), shell=True)
        # subprocess.check_call(r"DISPLAY=:1 xrandr --fb {width}x{height}x16".format(**env), shell=True)
